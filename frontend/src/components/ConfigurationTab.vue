<template>
  ...
  <div
    ...
    <p class="select-none mt-4 mx-4 text-md text-slate-100 font-semibold">Database</p>
    <CustomSelect
      v-model="form.database"
      class="relative mt-4 mx-4"
      :size="'full'"
      :options="getOptions.database(notionData)"></CustomSelect>
    <div
      ...
      <div class="flex flex-col col-span-1">
        <p class="select-none my-4 text-sm text-slate-100 font-medium">Calendar name</p>
        <CustomInput
          v-model="form.calendarName"
          :placeholder="form.database.name"
          class="mb-4 mr-4 h-8 w-auto"></CustomInput>
      </div>
      <div class="flex flex-col col-span-1">
    ...
    <div class="sticky buttom-0 flex grow w-full">
      <div class="flex flex-row absolute bottom-4 right-4 items-center">
        <CopyBlock v-if="successfulRequest" class="mx-4" :text_to_copy="calendarId"></CopyBlock>
        <div
          v-if="!successfulRequest"
          :class="{ 'animate-ping opacity-100': sending }"
          class="mx-4 size-3 rounded-full bg-green-500 opacity-0"></div>
        <CustomButton @click="createConnection()" :name="'Submit'" :color="'green'"></CustomButton>
      </div>
    </div>
  </div>
  <PopUp
...
const doneMethodProperty = computed(() => {
  const database = notionData.value.find(
    (result) => result.databaseId == form.value.database.value
  );
  if (typeof database == "undefined") return [];
  const doneMethodId = form.value.doneMethod.value;
...
const createConnection = async () => {
  try {
    if (!sending.value) {
      ...
      const res = await axios.post("http://localhost:6060/v1/api/connections", formToSend);

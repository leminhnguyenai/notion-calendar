<template>
  <nav
    ...
    <Cog8ToothIcon
      ...
      class="absolute right-6 h-6 text-[#606060] select-none cursor-pointer hover:text-[#d4d4d4] active:text-[#585858] active:duration-75 transition-all duration-200"></Cog8ToothIcon>
    <SyncStatus class="absolute left-6"></SyncStatus>
  </nav>
  <SettingPanel v-if="openSetting" @close="(close) => (openSetting = !close)"></SettingPanel>
  <div
    ...
    <ConnectionBlock
      ...
      :database_name="connection.database.name"></ConnectionBlock>
  </div>
  <div

...
const saveUserInfo = async (
    ...
    const user_id: string = await bcrypt.hash(
        `${refresh_token}_${email}_${new Date()}`,
        10,
    );
    await msgQueue.enqueue(
        pool.query(
            ...
            [user_id, email, refresh_token, refresh_token],
        ),
        'db',
    );
    await msgQueue.enqueue(
        pool.query('INSERT IGNORE INTO settings(user_id) VALUES(?)', [user_id]),
        'db',
    );
    await pool.end();

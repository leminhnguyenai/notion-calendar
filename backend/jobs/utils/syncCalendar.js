...
const syncCalendar = async (calendarId, notionEvents, googleCalEvents, relationTb) => {
  ...
  let copyNotionEvents = _.cloneDeep(notionEvents);
  let copyGoogleCalEvents = _.cloneDeep(googleCalEvents);
  let statistic = {
    taskAdded: 0,
    taskReAdded: 0,
    taskUpdated: 0,
  ...
  for (const notionEvent of copyNotionEvents) {
    let relation = copyRelationTb.find((result) => result.page_id == notionEvent.page_id);
    let googleCalEvent;
    if (relation) {
  ...
  for (const relation of copyRelationTb) {
    let googleCalEvent = copyGoogleCalEvents.find((result) => result.event_id == relation.event_id);

<template>
    <div
        class="flex flex-col size-2/5 bg-[#313131] rounded-md shadow-md overflow-y-auto"
    >
        <span
        ...
        <div class="sticky buttom-0 flex grow w-full">
            <div class="flex flex-row absolute bottom-4 right-4">
                <p
                    class="flex items-center justify-center relative mx-2 text-xs text-yellow-300"
                >
                    {{ warning_message }}
                </p>
                <CustomButton
...
const get_config_data = async () => {
    try {
        const res = await axios.get("http://localhost:6060/api/config");
        old_refresh_rate.value =
            JSON.parse(JSON.stringify(res)).data.refreshRate / 1000;
